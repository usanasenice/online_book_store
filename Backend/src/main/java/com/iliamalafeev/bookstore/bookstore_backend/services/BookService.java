...
import com.iliamalafeev.bookstore.bookstore_backend.entities.*;
import com.iliamalafeev.bookstore.bookstore_backend.repositories.*;
import com.iliamalafeev.bookstore.bookstore_backend.utils.ErrorsUtil;
import com.iliamalafeev.bookstore.bookstore_backend.utils.validators.BookValidator;
import com.iliamalafeev.bookstore.bookstore_backend.utils.validators.ReviewValidator;
...
//  <------------------------------------------------------------------------------->
    ...
    public Page<BookDTO> findAll(Integer page, Integer booksPerPage) {
        Pageable pageable = PageRequest.of(page, booksPerPage);
        Page<Book> books = bookRepository.findAllWithGenres(pageable);
    ...
    public BookDTO addBook(BookDTO bookDTO, BindingResult bindingResult) {
        ...
        if (bindingResult.hasErrors()) {
            ErrorsUtil.returnBookError("Some fields are invalid.", bindingResult, HttpStatus.BAD_REQUEST);
        }

        // Validate genres
        if (bookDTO.getGenres() == null || bookDTO.getGenres().isEmpty()) {
            ErrorsUtil.returnBookError("At least one genre must be selected", null, HttpStatus.BAD_REQUEST);
        }

        List<String> genreDescriptions = bookDTO.getGenres().stream()
                .map(GenreDTO::getDescription)
                .collect(Collectors.toList());

        List<Genre> genres = genreRepository.findByDescriptionIn(genreDescriptions);
        
        if (genres.isEmpty()) {
            ErrorsUtil.returnBookError("No valid genres found", null, HttpStatus.BAD_REQUEST);
        }

        if (genres.size() != genreDescriptions.size()) {
            List<String> foundGenres = genres.stream()
                    .map(Genre::getDescription)
                    .collect(Collectors.toList());
            List<String> missingGenres = genreDescriptions.stream()
                    .filter(desc -> !foundGenres.contains(desc))
                    .collect(Collectors.toList());
            ErrorsUtil.returnBookError("The following genres do not exist: " + String.join(", ", missingGenres), null, HttpStatus.BAD_REQUEST);
        }
    ...
    private Book convertToBook(BookDTO bookDTO) {
        ...
        if (bookDTO.getGenres() != null) {
            List<Genre> genres = bookDTO.getGenres().stream()
                    .map(genreDTO -> genreRepository.findByDescription(genreDTO.getDescription())
